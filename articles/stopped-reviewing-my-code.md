...
:::message
**先に言っておくと、この記事は「真似してもらう」ために書いています。**

出てくる設定はすべて、公開しているリポジトリで**実際に動いているもの**です。閾値ひとつ、コメント1行まで、その場の思いつきではなく理由があって今の値になっています。行番号付きのリンク（📎）を張ってあるので、**上から順に読む必要はありません**。

**使えそうなところだけコピーして持っていってください。同じ設定を入れれば、同じように動くはずです。**
:::
...
1つ強調しておきたいのですが、**ここに書いてあることは、基本的に全部 AI にやらせるためのものです。**

人間が読んで守るためのコーディング規約ではありません。「pure 関数に切り出せ」も「重複を消せ」も「テストが本当に落ちるか確かめろ」も、**書いた本人である僕がやることは想定していない**。エージェントに渡す指示です。

だから書き方も変わります。人間向けの規約なら「〜すること」で終わりますが、AI 向けなら**いつ読むか**（`Read before writing tests`）と**何をすればいいか**まで書き切る必要がある。行間を読んでくれることを期待した書き方は、一切通用しません。

逆に言うと、**人間が読んで守る前提を捨てた瞬間、規約はいくら細かくても構わなくなります**。この記事の後半に出てくる「人間なら耐えられない厳しさ」の話は、ここから始まっています。
...
なお、この章で出てきた ESLint と SonarJS を **CI にどう置くか**（何をゲートにして何をレポートにするか、既存の負債をどう返すか）は、[別記事に切り出してあります](https://zenn.dev/singularity/articles/clean-code-ci-for-ai-era)。設定を実際に入れる段になったら、そちらのほうが手順書として使えます。
...
そして念のため書いておくと、これは**僕が心がけていること**ではありません。**規約に書いてあって、エージェントがそう書きます。**

📎 [`CLAUDE.md#L188`](https://github.com/isamu/claude/blob/273144a8bf83ff5ea2b92d78ae908cdb2ab4b541/CLAUDE.md#L188) ／ 詳細は [`docs/testing.md#L36-L42`](https://github.com/isamu/claude/blob/273144a8bf83ff5ea2b92d78ae908cdb2ab4b541/docs/testing.md#L36-L42)

### 新しいコードだけの話ではない

ここが効きます。同じドキュメントに、こう書いてあります。
...
テスタブルであることは、**新しい行を縛るルールではなく、コードベースの性質として取り戻し続けるもの**だ、という立て方です。

なので定期的に、**コードベース全体を見直させて、切り出しとリファクタリングをやらせています**。既存の大きな関数の中に埋まっている pure なルールを引き剥がして、テストを付ける。**Claude Code の組み込みコマンド**である `/code-review` や `/simplify` を回すだけです（自作スキルではなく、最初から入っているものです）。
...
仕組みとしては、GitHub Actions で全 PR に Codex の自動レビューを走らせています。判定は決まったマーカーで返ってきます。
...
指摘の質は本当に良いのですが、オープンソースの無料プランで使っていたので、**1時間に1回しかレビューが回りません**。1日に数回なら十分だった枠が、**1時間に何本も PR が出るようになった瞬間に足りなくなりました。**

ここが致命的でした。レビューは1往復では終わりません。指摘をもらう → 直す → もう一度見てもらう。**この往復こそが本体**です。ところが1往復するたびに1時間待ち。3往復したら3時間です。
...
太平洋をまたいだ2人が、無料枠のリセットを黙って待っている。今思うと笑えます。**サブスク代なんて2人ともすぐ払えるのに、なぜかそれをせずに待っていた。**

さすがにこれは違うだろう、ということで、**まず手元で試しました**。Claude Code から Codex を呼び出すスキルを書いて、ローカルでレビューさせてみたんです。

これが、思ったより調子がよかった。**別のモデルに読ませるだけで、ちゃんと刺さる指摘が返ってくる**。
...
レビューの価値は1回の指摘ではなく、**往復の回数**にあります。だから1往復のコストが上がると、質そのものが落ちる。「待つのが面倒だから、この指摘は次でいいか」が始まった時点で終わりです。
...
なお CodeRabbit は、いまは **MulmoTerminal / MulmoClaude の両方で動いています**。Codex とは見ている角度が違うので、拾う問題も違う。**使えるなら使ったほうがいいです。**

レビュアーは、数を増やすというより**種類を増やす**のが効きます。同じ穴を3回見るより、違う穴を2回見るほうが価値がある。

### 空港でレビューしていた頃の話

もう1つ、2人しかいないチームには構造的な問題があります。**どちらかが旅行に出ると、人力のレビューが完全に止まる**ことです。

なので以前は、**空港や飛行機の中で PR をレビューしていました**。搭乗を待ちながら diff を開いて、機内の Wi-Fi でコメントを返す。今思うとクレイジーですが、当時はそれしか方法がなかった。レビューできる人間が2人しかいないというのは、そういうことです。

AI レビューが本当に効いたのは、まずここでした。**誰かが移動していても、レビューは止まらない。** 質がどうという以前に、**そこに常にいる**というだけで価値がありました。

そして、これがほんの半年前の話だということに、書いていて自分で驚いています。
...
複数走らせると、**普通に矛盾します**。Codex が「こう書け」と言い、CodeRabbit が逆を言う。両方を満たそうとすると、誰も読めないコードができあがります。
...
MIT です。使い方は[日本語ガイド](https://www.mulmoterminal.com/guide/ja/)にまとめてあります。1コマンドで試せるので、**いま複数のターミナルを行ったり来たりしている人は、一度だけ試してみてください。**
...
1. **別モデルのレビューを CI に入れる**（⑥）— 一番効きます。書いた側と読む側を別のモデルにするだけ。ワークフローを1本コピーするだけで、明日から指摘が流れてきます
2. **ESLint のサイズ・複雑さ・型の締め付け**（②）— コピペで入ります。既存コードが赤くなるはずなので、**まず warn で入れて、ゼロにしてから error に上げる**（記事中の「drain してから ratchet」）
3. **ルールを pure 関数に切り出してテスト**（③）— ここだけは設計の話なので時間がかかります。でも一番寿命が長い
4. **CI を3 OS に広げる**（⑤）— 手元にない環境を持つのは、これしか方法がありません

逆に、**最初にやらなくていい**のは重複・デッドコードのスキャン（④）です。あれは量が増えてから効くもので、初日から入れても「回避の作法」を覚えるだけでした。
