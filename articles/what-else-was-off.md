...
ちなみに `void` を禁じる `sonarjs/void-use` というルールがあり、これと**正面から矛盾**する ── と、公開時点では書いていました。

:::message alert
**訂正（2026-08-04）: 矛盾していませんでした。**

`sonarjs/void-use`（S3735）の実装は、**promise を除外**します。
...
つまり `no-floating-promises` が要求する `void <promise>` は、**最初から報告対象外**でした。有効に戻して測ったら、66件出ると思っていたものが **3件**。全部 `Map` メソッドの戻り値を捨てる `void` で、promise とは無関係でした。

**off にした理由が、そもそも成立していなかった**ことになります。→ [mulmoterminal#1362](https://github.com/receptron/mulmoterminal/issues/1362)

⚠️ ひとつ注意。**型情報を有効にしていないと**、このルールは「呼び出しっぽい式」を全部除外するフォールバックに落ちます。自分で試すなら先に `parserOptions.project` を通さないと、**間違ったことを確認して終わります。**
:::

**ルール同士がぶつかることもある** ── それ自体は本当ですが、**この例は違いました。** ぶつかっていたのは、私の理解のほうです。
...
そして**この記事を出したあとに、この記事の中身が1件間違っていた**ことが分かりました。上の「ルール同士が矛盾する」の節です（訂正済み）。

設定ファイルに書いた理由は、**間違っていても自信たっぷりのまま生き残ります。** 素の `eslint-disable` より信用されるぶん、質が悪い。その話は別記事にします。
...
4. **strict を入れても as は止まらない ── ESLint と tsconfig の「入れたつもり」を数える** ← **いまここ**
