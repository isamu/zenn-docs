...
### 起こしたルールは、指摘を読んでから残すか決める

そうやって起こした SonarJS のルールを、あとで実際に指摘を1件ずつ読んでいったところ、**4つは従うとコードが壊れる**とわかりました。件数を減らすために消したのではなく、このコードベースでは前提が成り立っていなかったので外した、という話です。

一番危なかったのが `no-alphabetical-sort` です。「`.sort()` は比較関数を渡せ」というルールで、狙いは `[10, 9, 1].sort()` が `[1, 10, 9]` になる事故。もっともな指摘に見えますが、24件の指摘の中にこれが混ざっていました。

```js
// Twilio: パラメータをキー順に並べ、key+value を連結し、URL を前置して署名する
const sortedKeys = Object.keys(params).sort();
```

Twilio の**リクエスト署名**です。サーバ側が同じ順序を再計算して検証するので、ここで求められているのは「ロケールに依存しない決まった順序」。ルールが薦める `localeCompare` は環境のロケールや ICU データで結果が変わるため、素直に従うと**署名が合わなくなって認証が落ちます**。ほかも frontmatter のキー、レコードID、日付入りのログファイル名と、決まった順序が必要な場所ばかりで、本来の標的である数値ソートは1件もありませんでした。

似た形でもう1つ、`different-types-comparison`（型の上で結果が変わらない比較を「不要」と指摘する）も外しました。17件すべてが、**型はそう言っているだけで実行時には本当に必要なガード**だったからです。

```ts
// JSON.stringify の型は string だが、undefined/関数/symbol を渡すと undefined が返る
if (serialised === undefined) return "";
// headers.get() はヘッダが無ければ null（これは Telegram の秘密トークン検証）
if (headerSecret !== env.TELEGRAM_WEBHOOK_SECRET) throw new Error(...);
// noUncheckedIndexedAccess は strict に含まれないので、
// Record<string, T> のインデックス読みは T 型になる（実行時は undefined が来る）
if (record[key] !== undefined) { ... }
```

3つ目が効きます。`noUncheckedIndexedAccess` は `strict: true` に**含まれない**ので、多くのプロジェクトは既定でオフです。その状態では「存在しないキーを読むと `undefined`」という当たり前の事実が型に現れず、実行時に必要なガードが型の上では「常に真」に見える。ルールはそれを不要と判断します。指示どおり消せば、次の行の `.length` が落ちるか、Webhook の認証チェックが消えます。

一般化するとこうなります。**ルールの指摘は「型が言っていること」であって「実行時に起きること」ではありません。** そして型は、設定次第で現実より狭くも広くもなります。だから新しいルールを有効にしたときは、件数だけ見て一括で直しにいかず、まず指摘を読む。読んだ結果「このコードベースでは前提が成り立たない」と分かったら、コードをルールに合わせて曲げるのではなく、ルールを外して**理由を設定ファイルに書き残す**。将来 `noUncheckedIndexedAccess` を入れたら `different-types-comparison` は再検討する価値が出る、というような判断が後から追えるようにしておきます。

AI に「lint の警告を全部直して」と投げるときも同じです。警告を消す作業は、消すこと自体が目的になった瞬間に危うくなります。Twilio の署名の件は、機械的に直していたら CI も型チェックも素通りして、本番で認証が落ちてはじめて気づくたぐいのものでした。
