...
:::message
この記事は「機械に守らせる」部分の実装編です。**なぜ人間のレビューをやめられたのか**という全体像は [1日500コミットは、もう読めない ── だからコードレビューをやめた](https://zenn.dev/singularity/articles/stopped-reviewing-my-code) に書いています。
:::
...
### 合わないルールは、コードではなく設定を直す

型情報ルールを入れると、指摘の中に「従うとコードが壊れる」ものが混ざります。ルールが間違っているというより、そのルールが前提にしている状況が自分のコードベースでは成り立たない、という形です。実際に外すことになったものを、そのまま設定例として置いておきます。

```js
rules: {
  // `.sort()` に比較関数を要求するルール。狙いは [10, 9, 1].sort() が
  // [1, 10, 9] になる事故だが、勧められる localeCompare はロケールと
  // ICU データで結果が変わる。API のリクエスト署名、frontmatter のキー、
  // ID、日付入りファイル名など「決まった順序」が要る場所では改悪になる。
  "sonarjs/no-alphabetical-sort": "off",
  // 型の上で結果が変わらない比較を不要と指摘するルール。
  // noUncheckedIndexedAccess が off だと実行時に必要なガードまで
  // 「常に真」に見えるので、消すと落ちる（下記参照）。
  "sonarjs/different-types-comparison": "off",
  // str.match(re) より re.exec(str) が速い、というルール。
  // 呼び出しのたびに主語と述語が入れ替わる割に、正しさは変わらない。
  "sonarjs/prefer-regexp-exec": "off",
}
```

2つ目に補足が要ります。`noUncheckedIndexedAccess`（存在しないキーを読んだら `undefined` かもしれない、と型に出す設定）は **`strict: true` に含まれない**ので、多くのプロジェクトで既定オフです。オフだと `Record<string, T>` のインデックス読みは `T` 型になり、実行時に必要なガードが型の上では「常に真」に見えます。

```ts
// noUncheckedIndexedAccess が off だと「常に真」扱い。だが実行時は undefined が来る
if (record[key] !== undefined) { ... }
// 型は string と言うが、undefined/関数/symbol を渡すと undefined が返る
if (JSON.stringify(value) === undefined) return "";
// headers.get() はヘッダが無ければ null（Webhook の署名検証などで効く）
if (request.headers.get(name) !== secret) throw new Error("...");
```

指摘どおりに消すと、次の行の `.length` が落ちたり、認証チェックが消えたりします。`noUncheckedIndexedAccess` を有効にすればこのルールの前提は成り立つので、そのときに再検討する価値はあります。

ここから引ける一般則は2つです。

**ルールの指摘は「型が言っていること」であって「実行時に起きること」ではない。** そして型は設定次第で現実より狭くも広くもなります。だから新しいルールを有効にしたら、件数を見て一括で直しにいく前に、まず指摘を読む。

**外すときは理由を設定ファイルに書く。** 上の例のように「なぜ合わないか」と「どうなったら再検討するか」を添えておくと、後から来た人が同じ調査をやり直さずに済みます。`eslint-disable` を何十個も撒くより、ルール1つを理由つきで切るほうが読めます。

AI に「lint の警告を全部直して」と投げるときも同じです。警告を消すこと自体が目的になると危うい。署名まわりの `.sort()` は、機械的に直せば lint も型チェックも CI も素通りして、本番で認証が落ちてはじめて気づくたぐいのものでした。
