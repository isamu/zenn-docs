---
title: "Claude Code と Codex を並列で動かす — MulmoTerminal 日本語ガイド"
emoji: "🖥️"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["ClaudeCode", "Codex", "vibecoding", "個人開発", "AI"]
published: true
publication_name: "singularity"
---

AI エージェントを何体も並列で動かすと、**どれが自分の返事を待っているのか分からなくなります**。ターミナルの中では「まだ考えている」も「終わった」も「許可待ちで止まっている」も、同じように文字が止まっているだけだからです。
...
この記事は、公式の [日本語ガイド](https://www.mulmoterminal.com/guide/ja/) を1ページにまとめたものです。
...
開くと**そのままグリッド**です（URL は `/terminals`）。1 体に集中する単一ビューは **`/chat`** に独立したアドレスがあるので、そちらから始めたい人はブックマークしてください。

サーバは既定で**ループバック（127.0.0.1）だけ**を待ち受けます。同じ LAN の別マシンから開きたいときは、明示的にアドレスを指定して起動してください。
...
**全セッションを 1 行ずつ並べたテキスト一覧**——が出ます（既定）。

各行が持っているのは、ディレクトリ・**AI 要約**・**直近のプロンプト**・**最新の返答**・状態語
（running / planning / done / idle …）、さらにブランチの **PR フェーズ**（draft / CI fail / changes /
ready / merged …）。**行をクリックすると拡大対象が切り替わります。**

拡大したまま、残り全員の「何をして、どこまで進んだか」が文字で追える——多数を回すときの主戦場です。

右上の **▤ / ☰** で、ロスターと**フィルムストリップ**（従来のサムネイル列）を行き来できます。**⤡** でグリッドへ戻ります。

各行の行数は `~/.mulmoterminal/config.json` の `cockpitLines` で変えられます（既定は サマリー 2 / プロンプト 2 / 返答 3）。
**サマリーを増やすと、拡大しなくても中身が読めます。**
...
さらに **OpenRouter などの Anthropic 互換バックエンド**を設定すれば、セッションごとにモデルを選べます
（Kimi / DeepSeek / GLM / Gemini / GPT / Grok など）。同梱の `claude-ollama` を使えば、**Ollama のモデルで
完全ローカル**に走らせることもできます。

### キーボードだけで盤面を回す

拡大・巡回・新規作成を、マウスに持ち替えずにできます。`~/.mulmoterminal/config.json` の `keymap` に割り当てます。
...
| アクション | 動き |
|---|---|
| **`zoom-toggle`** | 拡大 ↔ 折りたたみ。**カーソルのあるターミナル**が拡大され、戻ってもカーソルはそこに残る |
| **`next-attention`** | **見るべき次のターミナルへ**。要対応 → 完了して未レビュー → アイドル の順で、**ターン実行中は飛ばす**。端で止まらず巡回 |
| `zoom-next` / `zoom-prev` | 画面上の並び順に拡大対象を送る |
| `terminal-new` / **`terminal-new-adjacent`** | 新規／**隣に作ってディレクトリを継承**（分割に一番近い操作） |
| `terminal-close` | 閉じる |
| `copy` / `paste` | コピー・ペースト |

`next-attention` を押すたびに「自分待ち」を渡り歩けるので、**9 セルを目でスキャンする作業が消えます。**

:::message
**既定の割り当てはありません。** バインドしたキーは、ターミナルの中で動いているプログラムに届かなくなります。
そのトレードオフを決められるのは使う人だけなので、`keymap` を書くまでショートカットは OFF です。

書き間違えると**サーバが起動せず、該当行を表示して止まります**（黙って無視されると「効かないショートカット」と
区別がつかないため）。**Settings → Keyboard shortcuts** に、未割り当ても含めて全アクションが並びます。

macOS の注意: 最上段のキーはメディアキーなので `Fn` なしでは keydown が来ません。`Option`+英字は合成された文字として届きます。
`Cmd`/`Ctrl` + `W` / `T` / `N` はブラウザ予約なので割り当てられません。
:::

### ターミナルのファイルを、その場で見る・直す

ターミナルに出たファイルパスを**クリックすると、そのターミナルの隣にファイルが開きます**。

- コードはエディタ、`.md` はレンダリング済みビューへ、と**拡張子でルーティング**
- **ページをリロードしても開いたまま**
- **保存が、同じファイルを編集したエージェントを踏み潰しません**（ディスク側が変わっていたら保存前に知らせます）
- エディタが触ったファイルは **3 世代**残ります

「エージェントの出力を読んで、README だけ自分で直す」がグリッドから出ずに終わります。
...
> 音は「設定方法」で**好きな音声ファイル**に変えられます——ある開発者は、完了のたびに**銅鑼（ドラ）を「ゴーーン」と鳴らしている**とか。
...
**これが並列運用の効き方を一番変えます。** 席にいなくても、詰まったエージェントのほうから呼んでくれます。

- **Web Push** — タスクが**終わった**とき、または**入力待ちで止まった**ときに、スマホの**ロック画面**に通知が届きます
- **通知の種類を選べます** — 「終わった」は要らない、「止まって聞いている」だけ欲しい、が設定できます（`pushKinds`）
- **スマホから見て、答えられます** — セッションのライブ画面を見て、**yes / no / ok / continue / stop** をタップ、
  あるいは自分で書いた**クイック返信**を送れます
- **スマホから新しいターミナルを起動**できます（いま見ているセッションの作業ディレクトリで）
- リポジトリなら **GitHub へのリンク**も出ます

セットアップは 2 か所です。

1. **端末側** — ツールバーの **📱 RemoteHost** から **Connect（Google サインイン）**、
   さらに **Settings（⚙）→ Web Push notifications** を ON
2. **スマホ側** — **https://mulmoserver.web.app** を開いて同じ Google アカウントでサインイン →
   **「Enable notifications」**

:::message
**iPhone は「ホーム画面に追加」してから**開いてください。iOS では**ホーム画面にインストールした PWA からでないと
Web Push が使えません（Safari のタブからは有効にできません）。
:::
...
| **通知音を種類別に** | **「終わった」と「呼ばれている」で別の音**。それぞれ個別に ON/OFF（`soundKinds` / `sounds`） |
| **スマホへ Web Push** | ロック画面に届き、**その場で yes/no/continue を返せる**。種類も選べる（`pushKinds`） |
| **キーボード操作** | 拡大・巡回・新規作成をキーで（`keymap`・既定は空） |
...
```jsonc
{
  ...
  // キーボードショートカット（既定は空 = OFF）
  "keymap": { "zoom-toggle": "F1", "next-attention": "F2" },

  // 通知音: どの種類で鳴らすか / 種類ごとの音
  "soundKinds": ["waiting"],
  "sounds": { "waiting": "preset:chime" },

  // スマホへの Web Push: どの種類を飛ばすか
  "pushEnabled": true,
  "pushKinds": ["waiting"],

  // 拡大時のロスターの行数（既定 2 / 2 / 3）
...
| キー | 役割 |
|---|---|
| `cwdPresets` | ランチャの作業ディレクトリ補完（`{ label, path }` の配列） |
| `launchers` | グリッドセルの「OR LAUNCH」に並ぶ起動コマンド |
| `prRepos` | 横断 PR/Issue ビューの対象リポ |
| **`keymap`** | **キーボードショートカット。既定は空 = OFF**（→ 「キーボードだけで盤面を回す」） |
| **`soundKinds` / `sounds`** | **鳴らす種類**（`finished` / `waiting`）と、**種類ごとの音**。プリセット同梱 |
| **`pushEnabled` / `pushKinds`** | **スマホへの Web Push** の ON/OFF と、飛ばす種類 |
| **`cockpitLines`** | 拡大時のロスター1行あたりの行数（summary / prompt / reply） |
| `buttons` / `chips` | ヘッダーのボタン/チップ（プロジェクト設定とマージ。→ 「ヘッダーのカスタマイズ（ボタン / チップ）」） |

:::message
**設定ファイルを手で書かなくても大丈夫です。** Claude Code で **`/mulmoterminal-config`** と打つと、
対話でこのファイル（と `.mulmoterminal.json`）を作ってくれます。
:::
...
```jsonc
{
  "fontSize": 16,                              // 8〜32（既定 14）。範囲外はクランプ
  "fontFamily": "'JetBrains Mono', monospace", // 日本語を出すなら CJK 対応フォントを含める
  "orderPriority": 0                           // 小さいほどグリッドの前。本番リポを左上に固定できる
}
```

`fontSize` は Settings のステッパーからも変えられ、そちらは**ブラウザごとに記憶**されます（スマホとデスクトップで別々に持てます）。
ディレクトリ側に書くと、そのディレクトリの端末ではこちらが勝ちます。

:::message alert
**ブラウザのズームで代用しないでください。** xterm のグリッドとシェルが幾何を取り違え、カーソル位置と折り返しがずれます。
文字サイズは必ず `fontSize` で変えてください。
:::

`orderPriority` を書いていないディレクトリは、従来どおりの並びになります。

#### 使うモデル・バックエンド

```jsonc
{
  "provider": "openrouter",   // config.json の providers に定義した id
  "model": "z-ai/glm-5.2"     // このディレクトリのセッションが既定で使うモデル
}
```

「このリポジトリは安いモデルで十分」「ここだけ最上位モデル」をディレクトリ単位で決められます。
