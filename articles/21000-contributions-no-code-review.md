---
title: "7ヶ月で2万コントリビューション。でも僕はもうコードレビューをしていない"
emoji: "🚦"
type: "tech"
...
今年に入ってから今日まで、**7ヶ月間**の僕の GitHub の数字です（2026年1月1日〜7月26日）。
...
207日間で 21,003。1日あたり約100。**活動がゼロだった日は、207日のうち0日**でした。ピークは7月23日の767です。

そして、最後の行だけ桁がひとつ違います。

これは集計のミスではありません。**僕はもう、コードレビューをほとんどしていません**。場合によっては動作確認もしません。マージボタンを押すとき、僕はその diff を読んでいないことのほうが多い。

こう書くと無責任に聞こえると思います。実際、半年前の僕がこれを読んだら「そんなの事故るに決まってる」と言ったはずです。

でも、事故っていないのです。簡単なバグはほぼ出ません。

なぜかというと、**レビューでしか捕まらないものを、先に減らしたから**です。この記事は、その「先に減らす」ために何を整えたかの話です。

:::message
念のため補足すると、GitHub の「レビュー」は他人の PR へのレビューを数えます。なので 387 という数字がそのまま「自分の PR を読んでいない証拠」になるわけではありません。ただ、体感としても実態としても、**僕が自分の diff を読む時間は限りなくゼロに近づいています**。
:::
...
（数字の整合について先に言っておくと、上の表のコミット13,073は GitHub がデフォルトブランチへのコミットとして数えたぶんで、7ヶ月ならすと1日63です。「1日500」はピーク日の話。**同じ活動でも数え方でいくらでも変わる**ので、この手の数字は話半分に見てください。僕自身、指標として信用していません）
...
意外に思われるかもしれませんが、mulmoterminal の `CLAUDE.md` は **46行しかありません**。
...
「OK が出るまで繰り返す」というのは、つまりこの分類を繰り返すということです。ここだけは、まだ人間の判断が要ります。
...
7ヶ月で 21,003 という数字は、**頑張った結果ではありません**。むしろ逆で、頑張らなくても壊れないようにした結果です。

そして念のため先に言っておくと、**コミット数もコントリビューション数も、生産性の指標としてはゴミです**。僕は細かくコミットするので、粒度の指標にしかなりません。超人的なエンジニアになったわけでもありません。
...
この記事で紹介した設定は、全部この2つのリポジトリで動いています。MIT なので、そのまま持っていってください。

- [receptron/mulmoterminal](https://github.com/receptron/mulmoterminal) — `eslint.config.js` / `.github/workflows/` / `CLAUDE.md`
