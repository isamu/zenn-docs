---
title: "あなたの tsconfig と ESLint、本当にエラーになりますか ── 貼ると必ず落ちるサンプル19本"
emoji: "🧪"
type: "tech"
...
これ、人類には早すぎると思っています。**スレッドプログラミングと同じ種類の難しさ**です。一つ一つは理解できるのに、組み合わさった瞬間に、自分の頭の中のモデルと実際の挙動がずれる。しかもずれても、**何も言われない。**

前回の記事で、規約に「`as` を使うな」と書いてあるのに `as` が149箇所あった話を書きました（→ [前回](https://zenn.dev/singularity/articles/what-else-was-off)）。原因は単純で、**ESLint で禁止していなかった**からです。

そのとき思ったのが、これです。

> **設定が入っているかどうかを、目で確かめる方法がない。**

設定ファイルを読んでも分かりません。プリセットが継承されるからです。`strict` を入れたからといって、何が有効になったのかはどこにも書いていません。

なので、**理解するのをやめました。** 代わりに、**貼ると必ずエラーになるコード**を用意しました。

エラーが出れば効いています。出なければ入っていません。それだけです。**設定を理解していなくても、効いているかどうかは分かります。**
...
そして最初の話に戻ります。**設定を全部理解している必要はありません。**

TypeScript も Node も ESLint も、追いつけない速さで変わり続けます。7.0 が出たのは先月です。全部把握してから設定を書ける人が、どれだけいるでしょうか。

でも「**効いているかどうか**」は、理解していなくても確かめられます。貼って、走らせて、落ちるかを見るだけなので。

**理解と確認は別の作業です。** 確認のほうは、今日できます。
...
5. **あなたの tsconfig と ESLint、本当にエラーになりますか ── 貼ると必ず落ちるサンプル19本** ← **いまここ**
