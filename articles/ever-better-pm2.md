...
ただ、その足回りは**自分のリポジトリだから最初から入れられた**だけです。
...
そして linter の設定には、**「多すぎて今は直せない」**と書いて `no-explicit-any` が off にしてありました。
...
- **1日500コミットには足回りが要る。** でもそれは自分のリポジトリだから入れられただけで、**すでにあるコードには入っていない**
