...
天井を凍結する道具にしたのは、まさにそこが理由です。**「全部直せ」ではなく「これ以上増やすな」**なら、今日から成立します。
...
**天井が 4,933 → 3,637 に下がりましたが、そのうち 1,292 件は「直した」のではありません。** `@typescript-eslint/no-require-imports` を `**/*.js` で対象外にしたぶんです。CommonJS のパッケージで `.js` ファイルが `import` を使えない以上、このルールは**新しいファイルを全部止めるだけで、線を守っていませんでした。**
